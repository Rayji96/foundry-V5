...
Hooks.once("init", async function () {
  ...
  Handlebars.registerHelper(
    ...
    function (willpowerMax, willpowerAgg, willpowerSup) {
      ...
      const willpowerDice = Math.max(
        willpowerMax - willpowerAgg - willpowerSup,
        1
      );
  ...
  Handlebars.registerHelper("visibleDisciplines", function (disciplines) {
    return Object.keys(disciplines).reduce((obj, key) => {
  ...
  Handlebars.registerHelper("visibleEdges", function (edges) {
    return Object.keys(edges).reduce((obj, key) => {
...
Hooks.on("renderSidebarTab", (app, html) => {
  ...
  const ownedCharacters = Array.from(game.actors).filter(
    (c) => c.permission === OWNED_PERMISSION
  );
  const options = {
      ...
      renderTemplate(template, options).then((c) => {
        if (c.length > 0) {
          ...
          prepareSearchableSelection(
            ...
            $content,
            options,
            (event) => game.actors.get(event.target.value)
          );
          ...
          options.pool1 =
            options.pool1 &&
            $content.find(`#pool1 option[value=${options.pool1}]`).length > 0
              ? options.pool1
              : $content.find("#pool1 option").attr("value");
          prepareSearchableSelection(
            "pool2",
            $content,
            options,
          ...
          options.pool2 =
            options.pool2 &&
            $content.find(`#pool2 option[value=${options.pool2}]`).length > 0
              ? options.pool2
              : $content.find("#pool2 option").attr("value");
...
function dieSelect() {
  ...
  if (
    ...
    $(".willpower-reroll .selected").length < 3
  ) {
...
function rerollDie(roll) {
  ...
  if (diceSelected > 0 && diceSelected < 4 && charactertype !== "hunter") {
    rollDice(
      diceSelected,
      speaker,
      game.i18n.localize("VTM5E.WillpowerReroll"),
      0,
      false,
      false,
      true
