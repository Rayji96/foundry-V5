...
export class VampireActorSheet extends ActorSheet {
    ...
    _onRoll(event) {
        ...
        if (dataset.roll) {
            ...
            roll_result.terms[0].results.forEach((dice,) => {
                if (dice.success) {
                    if (dice.result == 10) {
                        crit_success++;
                    } else {
            ...
            for (let i = 0, j = crit_success; i < j; i++) {
    ...
    _vampireRoll(num_dice, actor, label = "", difficulty = 0) {
        ...
        roll_result.terms[2].results.forEach((dice,) => {
            if (dice.success) {
                if (dice.result == 10) {
                    hunger_crit_success++;
                } else {
