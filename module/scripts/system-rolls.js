...
import { _subtractWillpower } from './rolls/willpower-damage.js'
...
class WOD5eDice {
   ...
   * @param subtractWillpower         (Optional, default false) Whether to subtract willpower after the roll is complete
  ...
  static async Roll({
    ...
    const _roll = async (inputBasicDice, inputAdvancedDice, $form) => {
      ...
      const rollFormula = await generateRollFormula({ basicDice: inputBasicDice, advancedDice: inputAdvancedDice, system, actor, data, rerollHunger })
      ...
      const content = await generateRollMessage({ system, roll, actor, data, title, flavor })
        ...
        new Dialog(
          {
            ...
            close: html => {
              ...
              if (subtractWillpower && game.settings.get('vtm5e', 'automatedWillpower')) _subtractWillpower(actor)
              resolve(roll)
            }
          },
          {
